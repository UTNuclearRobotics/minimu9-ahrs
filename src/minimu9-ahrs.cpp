...
void ahrs(IMU & imu, fuse_function * fuse, rotation_output_function * output)
{
    ...
    while(ros::ok())
    {
        ...
        magnetic_field_raw.magnetic_field.x = imu.raw_m[0];
        magnetic_field_raw.magnetic_field.y = imu.raw_m[1];
        magnetic_field_raw.magnetic_field.z = imu.raw_m[2];

        sensor_msgs::Imu imu_msg_raw;
        imu_msg_raw.header.stamp = generated_message_time;
        imu_msg_raw.header.frame_id = "imu_link";
        imu_msg_raw.linear_acceleration.x = imu.raw_a[0];
        imu_msg_raw.linear_acceleration.y = imu.raw_a[1];
        imu_msg_raw.linear_acceleration.z = imu.raw_a[2];

        imu_msg_raw.angular_velocity.x = imu.raw_g[0];
        imu_msg_raw.angular_velocity.y = imu.raw_g[1];
        imu_msg_raw.angular_velocity.z = imu.raw_g[2];
