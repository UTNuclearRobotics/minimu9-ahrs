...
void ahrs(std::string bag_path, fuse_function * fuse, rotation_output_function * output)
{
    rosbag::Bag bag;
    double start; // truncate 64-bit return value
    bag.open(bag_path.c_str(), rosbag::bagmode::Read);
    std::vector<container> gyro_readings_for_offset;
    std::cout<< "OPENED" ;
    
    std::vector<std::string> topics;
    topics.push_back(std::string("/dn1/imu_chatter"));

    rosbag::View view(bag, rosbag::TopicQuery(topics));
    int message_index = 0;
    foreach(rosbag::MessageInstance const msg, view)
    {
      sensor_msgs::Imu::ConstPtr reading = msg.instantiate<sensor_msgs::Imu>();
      ...
      {
        if (message_index == 0)
          start = reading->header.stamp.toSec();
        else if (message_index == 32)
          break;
        message_index++;
        container r;
        r.reading[0] = reading->angular_velocity.x;
        r.reading[1] = reading->angular_velocity.y;
        r.reading[2] = reading->angular_velocity.z;
        gyro_readings_for_offset.push_back(r);
      }
    }
    bag.close();
    
    loadCalibration();
    measureOffsets(gyro_readings_for_offset);
    ...
    foreach(rosbag::MessageInstance const msg, view2)
    {
        ...
        if (reading != NULL)
        {
            double last_start = start;
            start = reading->header.stamp.toSec();
            float dt = (start-last_start);
            if (dt < 0){ throw std::runtime_error("Time went backwards."); }
            container g;
            g.reading[0] = reading->angular_velocity.x;
            g.reading[1] = reading->angular_velocity.y;
            g.reading[2] = reading->angular_velocity.z;

            vector angular_velocity = readGyro(g);
            
            container a;
            a.reading[0] = reading->linear_acceleration.x;
            a.reading[1] = reading->linear_acceleration.y;
            a.reading[2] = reading->linear_acceleration.z;
            vector acceleration = readAcc(a);
            
            container m;
            a.reading[0] = reading->orientation.x;
            a.reading[1] = reading->orientation.y;
            a.reading[2] = reading->orientation.z;
            vector magnetic_field = readMag(m);
...
int main(int argc, char *argv[])
{
    try
    {
        ...
        std::string mode, output_mode, i2cDevice, bag_path;
        ...
        desc.add_options()
            ...
            ("bag", opts::value<std::string>(&bag_path)->default_value("test.bag"),
             "specifies the bag file from where to read.\n")
            ;
        ...
        {
            ahrs(bag_path, &fuse_gyro_only, output);
        }
        else if (mode == "compass-only")
        {
            ahrs(bag_path, &fuse_compass_only, output);
        }
        else if (mode == "normal")
        {
            ahrs(bag_path, &fuse_default, output);
        }
        else
        {
